...
## v4.0.3 방별 상태 분리 수정
- 같은 브라우저에서 GOCHU1 → GOCHU3로 이동해도 대상 방의 DB 상태를 강제로 새로 불러옵니다.
- 이전 방의 대기 중 자동 저장이 새 방에 기록되지 않도록 저장 작업에 방 코드와 비밀번호를 고정했습니다.
- 온라인 방끼리 이동할 때 로컬 경매 저장본을 덮어쓰지 않습니다.
- 연결 해제 시 온라인 방 상태 대신 기존 로컬 경매 상태를 복원합니다.
- 새 방 만들기는 현재 온라인 방 진행 상황을 복사하지 않고 선수/설정만 사용해 초기 경매 상태로 생성합니다.
